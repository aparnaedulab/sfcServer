...
module.exports = function(sequelize, DataTypes) {
  var User_Course_Enrollment_Detail_Convo = sequelize.define("User_Course_Enrollment_Detail_Convo", {
    ...
    sequelize,
    tableName: 'User_Course_Enrollment_Detail_Convo',
    timestamps: true,
  ...
  // {
  //   classMethods: {
    User_Course_Enrollment_Detail_Convo.assigndata = function(user_id){
          var query='';
          ...
          User_Course_Enrollment_Detail_Convo.getAlldata = function() {
        var query = 'Select * from User_Course_Enrollment_Detail';
        // 
        return sequelize.query(query, { type: sequelize.QueryTypes.SELECT});
        };
        User_Course_Enrollment_Detail_Convo.getListLastData = function() {
          var query='';
          ...
          query +=" SELECT id , enrollment_no From User_Course_Enrollment_Detail_Convo ";
          query +=" WHERE id=(SELECT MAX(id) FROM User_Course_Enrollment_Detail_Convo)";
  ...
  // User_Course_Enrollment_Detail_Convo.belongsTo(sequelize.models.User, {foreignKey: 'user_id'});
  // User_Course_Enrollment_Detail_Convo.belongsTo(sequelize.models.Application, {foreignKey: 'application_id'});

      User_Course_Enrollment_Detail_Convo.getApplicationDetailsById = function(app_id){
        ...
        query += " FROM Application AS app JOIN User_Course_Enrollment_Detail_Convo AS ued ON ued.application_id = app.id ";
        query += " WHERE app.id = " + app_id;
        return sequelize.query(query, { type: sequelize.QueryTypes.SELECT});
      }
  
  return User_Course_Enrollment_Detail_Convo;
};